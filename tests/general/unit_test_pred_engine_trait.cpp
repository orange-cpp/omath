...
TEST(PredEngineTrait, PredictProjectilePositionBasic)
{
    ...
    const auto pos = PredEngineTrait::predict_projectile_position(p, /*pitch*/0.f, /*yaw*/0.f, /*time*/1.f, /*gravity*/9.81f);
...
{
    const Vector3<float> d{3.f,4.f,0.f};
...
TEST(PredEngineTrait, CalcViewpointFromAngles)
{
    ...
    const Vector3<float> predicted{10.f, 0.f, 0.f};
    const std::optional<float> pitch = 45.f;
...
{
    const Vector3<float> origin{0.f,0.f,0.f};
    const Vector3<float> target{0.f,1.f,1.f};
