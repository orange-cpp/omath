...
TEST(LineTracerExtra, MissParallel)
{
    ...
    constexpr Ray ray{ .start={0.3f,0.3f,1.f}, .end={0.3f,0.3f,2.f}, .infinite_length=false }; // parallel above triangle
...
{
    constexpr Triangle<Vector3<float>> tri({0,0,0},{1,0,0},{0,1,0});
    constexpr Ray ray{ .start={0.3f,0.3f,-1.f}, .end={0.3f,0.3f,1.f}, .infinite_length=false };
    const auto hit = LineTracer::get_ray_hit_point(ray, tri);
...
{
    constexpr Triangle<Vector3<float>> tri({0,0,0},{1,0,0},{0,1,0});
    constexpr Ray ray{ .start={0.0f,0.0f,1.f}, .end={0.0f,0.0f,0.f}, .infinite_length=false };
...
TEST(LineTracerExtra, InfiniteRayIgnoredIfBehind)
{
    ...
    constexpr Ray ray{ .start={0.5f,0.5f,-1.f}, .end={0.5f,0.5f,-2.f}, .infinite_length=true };
    const auto hit = LineTracer::get_ray_hit_point(ray, tri);
    EXPECT_EQ(hit, ray.end);
}
